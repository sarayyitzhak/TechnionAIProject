...
class ShowHyperParamsAnalysis:
    ...
    def show_hyper_params_analysis(self):
        ...
        fig.suptitle(f"{'Train' if self.y_axis_type == 'TRAIN' else 'Validation'} Accuracies by hyper-params")
        for idx, min_samples_leaf in enumerate(min_samples_leaf_groups.groups.keys()):
            ...
            for min_for_pruning in min_for_pruning_groups.groups.keys():
                ...
                graph.plot(data_by_min_for_pruning["max depth"], data_by_min_for_pruning[f"{'train' if self.y_axis_type == 'TRAIN' else 'valid'} score"], '.-', label=str(min_for_pruning))
